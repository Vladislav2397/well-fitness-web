...
const generateByShared = folderNames => {
  return folderNames.map(name => `../src/shared/ui/${name}/**/*.stories.(js|jsx|ts|tsx)`);
};
const generateByEquipment = folderNames => {
  return folderNames.map(name => `../src/entities/equipment/ui/${name}/**/*.stories.(js|jsx|ts|tsx)`);
};
module.exports = {
  stories: [...generateByShared(['Price', 'Quantity', 'Rating'])
